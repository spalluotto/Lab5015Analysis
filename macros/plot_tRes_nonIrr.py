...
for bar in range(0,16):
    ...
    c.SetGridy()
    c.cd()
    hdummy = ROOT.TH2F('hdummy_%d'%(bar),'',100,0,3E09,100,0,45)
...
c.SetGridy()
c.cd()    
hdummy = ROOT.TH2F('hdummy','',16, 0.0, 4.0 ,100, 0,45)
hdummy.GetXaxis().SetTitle('V_{OV}^{eff} [V]')
hdummy.GetYaxis().SetTitle('slew rate at the timing thr. [#muA/ns]')
hdummy.Draw()
for sipm in sipmTypes:
...
for sipm in sipmTypes:
    for ov in Vovs[sipm]:
        ...
        hdummy = ROOT.TH2F('hdummy_%s_%.2f'%(sipm,ov),'',100,-0.5,15.5,100,0,45)
...
for sipm in sipmTypes:
    for ov in Vovs[sipm]:
        ...
        c.SetGridy()
        c.cd()
        hdummy = ROOT.TH2F('hdummy_%s_%.2f'%(sipm,ov),'',100,0,45.5,100,0,60)
        ...
        c.SetGridy()
        c.cd()
        hdummy = ROOT.TH2F('hdummy_%s_%.2f'%(sipm,ov),'',100,0,45,100,0,60)
        hdummy.GetXaxis().SetTitle('slew rate at the timing thr. [#muA/ns]')
