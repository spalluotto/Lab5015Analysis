...
for par in pars_to_scale:
    ...
    for i in range(0, g[par].GetN()):
        vov = g[par].GetX()[i]
        #s_noise = g_Noise[par].Eval(vov)/enScale
        sr = g_SR[par].Eval(vov)
        s_noise =  sigma_noise(sr*enScale, '2c')
        s_stoch = g_Stoch[par].Eval(vov)/math.sqrt(enScale)
        s_dcr = 0.
        ...
        g_scaled[par].SetPointError(i, 0, g[par].GetErrorY(i)/enScale) # correct for angle offset
