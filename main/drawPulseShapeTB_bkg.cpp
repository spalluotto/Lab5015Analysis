...
int main(int argc, char** argv)
{
  ...
  for(auto mapIt : h1_energyLR){
    ...
    float energy_min = 5;
    float energy_max = 900;

    TSpectrum* spectrum = new TSpectrum(5);
    histo->GetXaxis()->SetRangeUser(energy_min, energy_max);
    int nFound = spectrum->Search(histo, 2.0, "nodraw", 0.005);
        
    // -- bkg

    TH1F* hBack = (TH1F*)spectrum->Background(histo,50, "same");
    TH1F* hDiff = (TH1F*)histo->Clone() ;
    hDiff->Add(hBack, -1.) ;
    hDiff->GetXaxis()->SetRangeUser(energy_min, energy_max) ;
    histo->GetXaxis()->SetRangeUser(0, 950);  
          
    // --- search the highest peak ---
    TSpectrum* diff_spectrum = new TSpectrum(2) ;
    int diff_nFound   = diff_spectrum->Search(hDiff, 6, "", 0.005)  ;
    double* diff_peaks    = diff_spectrum->GetPositionX() ;
    std::vector<float> peakMax      {  99,-99, -99 }  ;

    for(int ip = 0; ip < diff_nFound; ip++)
      {
	float x = diff_peaks[ip];
	float y = histo->GetBinContent(histo->FindBin(diff_peaks[ip])) ;
	if (y > peakMax[2])
	  {
	    peakMax[0] = ip ;
	    peakMax[1] = x  ;
    ...
    std::vector<float> peakMax2      {  99,-99, -99 }  ;
    for(int ip = 0; ip < diff_nFound; ip++)
      {
	if(ip == peakMax[0])
	  continue;
    ...
    if (peakMax2[1] > energy_min)
      {
	for(int i = 0; i<3 ; i++)
	  peakMax[i] = peakMax2[i];
      }
    if(peakMax[1] < energy_min)
      peakMax[1] = energy_min;

    TF1 *f_landau = new TF1(Form("f_landau_bar%02d_Vov%.2f_th%02d",iBar,Vov,ith),"[0]*TMath::Landau(x,[1],[2])", 0,1000.);                 
    float xmin = peakMax[1] * 0.65;                                                                                                                                              
    float xmax = std::min(peakMax[1]*2.5, 940.);
    f_landau -> SetRange(xmin,xmax);                                                                                                                               
    f_landau -> SetParameters(histo->Integral(histo->GetMaximumBin(), histo->GetNbinsX())/10, peakMax[1], 0.1*peakMax[1]);                                                       
    ...
    if (  f_landau->GetParameter(1) > energy_min &&                                                                                              
	  (f_landau->GetParameter(1) - 2.0 * std::abs(f_landau->GetParameter(2))) >= energy_min &&
