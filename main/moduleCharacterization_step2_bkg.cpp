...
// -- if MIP peak, we don't use the spectrum analyzers - just langaus fit to the energy peak.
	  ...
	  if(!source.compare(TB)){ 
	    ...
	    std::cout <<"\n\n";
	    std::cout <<"bar: " <<iBar <<"     vth1: " <<vth1 <<std::endl;
	    ...
	    // ---- landau fit around the maximum peak -----	    	    
	    ...
	    f_landau[index] -> SetParameters(histo->Integral(histo->GetMaximumBin(), histo->GetNbinsX())/10, peakMax[1], 0.1*peakMax[1]);
